import {
	sizenames,
	named,
	rule,
	group,
	vars,
	times,
} from "../js/uicss.js";
import { colormix } from "./colors.js";

const hrColor = colormix(
	vars.border.color.base,
	vars.border.color.tint,
	vars.border.color.blend,
	vars.border.color.opacity,
);

export default named({
	headings: group(
		...times(7, (si) =>
			rule(
				[
					...times(7, (hi) => `h${hi + 1}.sz-${si}.t`),
					...times(7, (hi) => `.t h${hi + 1}.sz-${si}`),
				],
				{
					font_size: `${vars.heading.size[si]}`,
					line_height: vars.heading.line,
				},
			),
		),
		// .hX rules do not have padding
		...times(7, (i) =>
			rule(`.h${i + 1}`, {
				font_size: `${vars.heading.size[6 - i]}`,
				line_height: vars.heading.line,
				font_weight: vars.font.weight.bold.or("600"),
			}),
		),
		// hX.t / .t hX have padding/margin
		...times(7, (i) =>
			rule([`h${i + 1}.t`, `.t h${i + 1}`], {
				font_size: `${vars.heading.size[6 - i]}`,
				line_height: vars.heading.line,
				font_weight: vars.font.weight.bold.or("600"),
				margin_top: `${vars.margin[2]}`,
				margin_bottom: `${vars.margin[2]}`,
				padding_bottom: `${vars.pad[0]}`,
			}),
		),
	),
	paragraphs: group(
		rule(["a.link", "a.t", ".t a[href]"], {
			color: vars.color.link,
			text_decoration: "underline",
		}),
		rule(["a.link:hover", "a.t:hover", ".t a[href]:hover"], {
			text_decoration_thickness: "2px",
		}),
		rule(["p.t", ".t p"], {
			margin_top: `${vars.text.gap}`,
			margin_bottom: `${vars.text.gap}`,
			line_height: `${vars.text.line.height}`,
		}),
		rule(["p.t:first-child", ".t p:first-child"], {
			margin_top: "unset",
		}),
		rule(["p.t:last-child", ".t p:last-child"], {
			margin_bottom: "unset",
		}),
	),
	inline: group(
		rule([".t em", "em.t", ".t i", "i.t"], {
			font_style: "italic",
		}),
		rule([".t s", "s.t"], {
			text_decoration: "line-through",
		}),
		rule([".t strong", "strong.t", ".t b", "b.t"], {
			font_weight: "bold",
		}),
		rule([".t code", "code.t"], {
			font_family: vars.font.code.family,
			font_size: "0.875em",
			padding: "0.1em 0.3em",
			background_color: vars.text.code.background,
			border_radius: vars.text.code.radius,
		}),
		rule([".t small", "small.t"], {
			font_size: "0.75em",
		}),
		rule([".t a", "a.t"], {
			color: "inherit",
			text_decoration: "underline",
		}),
		rule([".t a:hover", "a.t:hover"], {
			opacity: 0.75,
		}),
		rule([".t sub", "sub.t"], {
			vertical_align: "sub",
			font_size: `${vars.text.inline.subsup_size}`,
		}),
		rule([".t sup", "sup.t"], {
			vertical_align: "super",
			font_size: `${vars.text.inline.subsup_size}`,
		}),
		rule([".t u", "u.t"], {
			text_decoration: "underline",
		}),
	),
	blockquote: group(
		rule([".t blockquote", "blockquote.t"], {
			border_left: `${vars.text.blockquote.border.width} solid`,
			border_color: "currentColor",
			margin_left: "0",
			margin_right: "0",
			margin_top: `${vars.text.gap}`,
			margin_bottom: `${vars.text.gap}`,
			padding_left: `${vars.text.blockquote.padding.horizontal}`,
			padding_top: `${vars.text.blockquote.padding.vertical}`,
			padding_bottom: `${vars.text.blockquote.padding.vertical}`,
			opacity: `${vars.text.blockquote.opacity}`,
		}),
	),
	code: group(
		rule([".t pre", "pre.t"], {
			font_family: vars.font.code.family,
			background: `${vars.text.code.background}`,
			border_radius: `${vars.text.code.radius}`,
			margin_top: `${vars.text.gap}`,
			margin_bottom: `${vars.text.gap}`,
			padding: `${vars.text.code.padding.vertical} ${vars.text.code.padding.horizontal}`,
			overflow_x: "auto",
			white_space: "pre",
			contain: "paint",
		}),
		rule([".t pre code", "pre.t code"], {
			display: "block",
			max_width: "100%",
			white_space: "pre-wrap",
			overflow_wrap: "anywhere",
			background: "transparent",
			padding: "0",
			font_size: "1em",
			border_radius: "0",
		}),
	),
	lists: group(
		rule(["ul.t", "ol.t", "dl.t", ".t ul", ".t ol", ".t dl"], {
			margin_top: `${vars.text.gap}`,
			margin_bottom: `${vars.text.dd.margin.bottom}`,
			padding_left: `${vars.text.list.unordered.indent}`,
		}),
		rule(["ol.t", ".t ol"], {
			padding_left: `${vars.text.list.ordered.indent}`,
		}),
		rule(["li.t > p", ".t li p"], {
			margin_top: "unset",
			margin_bottom: "unset",
		}),
		rule(["ul.t li", ".t ul li"], {
			list_style_type: "disc",
		}),
		rule(["ol.t li", ".t ol li"], {
			list_style_type: "decimal",
		}),
		rule([".t li", "li.t"], {
			margin_bottom: `${vars.text.list.item.gap}`,
		}),
	),
	definition: group(
		rule([".t dl", "dl.t"], {
			padding_left: "0em",
		}),
		rule([".t dt", "dt.t"], {
			margin_top: `${vars.text.dt.margin.top}`,
			margin_bottom: `${vars.text.dt.margin.bottom}`,
			font_weight: "bold",
			opacity: `${vars.text.dt.opacity}`,
		}),
		rule([".t dd", "dd.t"], {
			margin_top: `${vars.text.dd.margin.top}`,
			margin_bottom: `${vars.text.dd.margin.bottom}`,
		}),
	),
	whitespace: group(
		rule(".nobreak", { white_space: "nobreak" }),
		rule(".nowrap", { white_space: "nowrap" }),
		rule(".break", { overflow_wrap: "break-word" }),
		rule(".wrap", { text_wrap: "wrap" }),
		rule(".noheading", {
			__heading_base: 1,
			__heading_max: 1,
			__heading_min: 1,
			__heading_amplitude: 0,
			__heading_unit: "1rem",
			// FIXME: In theory, these should not be required, but
			// it doesn't seem that the sizes recalculate.
			__heading_size_0: "1rem",
			__heading_size_1: "1rem",
			__heading_size_2: "1rem",
			__heading_size_3: "1rem",
			__heading_size_4: "1rem",
			__heading_size_5: "1rem",
			__heading_size_6: "1rem",
		}),
		rule(".pre", { white_space: "pre" }),
		rule(".pre-lines", { white_space: "pre-line" }),
		rule(".ellipsis", { text_overflow: "ellipsis", overflow: "hidden" }),
	),
	transform: group(
		rule([".caps", ".upper"], { text_transform: "uppercase" }),
		rule(".lower", { text_transform: "lowercase" }),
		rule(".cap", { text_transform: "capitalize" }),
	),
	alignment: group(
		rule(".center:not(.row), .t-center", { text_align: "center" }),
		rule(".right, .t-right", { text_align: "end" }),
		rule(".left, .t-left", { text_align: "start" }),
		rule(".justify, .t-justify", { text_align: "justify" }),
		rule(".middle:not(.row), .v-m", { vertical_align: "middle" }),
		rule(".baseline, .v-bl", { vertical_align: "baseline" }),
		rule(".top, .v-t", { vertical_align: "top" }),
		rule(".text-top, .v-tt", { vertical_align: "text-top" }),
		rule(".bottom, .v-bt", { vertical_align: "bottom" }),
	),
	direction: group(
		rule(".t-rtl", {
			direction: "rtl",
			text_align: "inherit",
		}),
		rule(".t-ltr", {
			direction: "ltr",
		}),
	),
	delimiter: group(
		rule(".sep-path>*:after", {
			display: "inline-block",
			padding_left: vars.gap,
			content: '"/"',
		}),
		rule(".sep-comma>*:after", {
			display: "inline-block",
			padding_right: vars.gap,
			content: '", "',
		}),
		rule(".sep-dash>*:after", {
			display: "inline-block",
			padding_left: vars.gap,
			content: '"―"',
		}),
		rule(".sep-dot>*:after", {
			display: "inline-block",
			padding_left: vars.gap,
			content: '"·"',
		}),
		rule(
			[
				".sep-path>*:last-child:after",
				".sep-comma>*:last-child:after",
				".sep-dash>*:last-child:after",
				".sep-dot>*:last-child:after",
			],
			{ display: "none" },
		),
	),
	decorations: named({
		parens: group(
			rule(".parens:before", { content: '"("' }),
			rule(".parens:after", { content: '")"' }),
		),
		brackets: group(
			rule(".brackets:before", { content: '"{"' }),
			rule(".brackets:after", { content: '"}"' }),
		),
		sqbrackets: group(
			rule(".sqbrackets:before", { content: '"["' }),
			rule(".sqbrackets:after", { content: '"]"' }),
		),
	}),
	hr: group(
		rule(".hr", {
			display: "flex",
			align_items: "center",
			justify_content: "center",
			text_align: "center",
		}),
		rule([".hr::before", ".hr::after"], {
			content: "''",
			flex_grow: 1,
			height: vars.border.width.or("1px"),
			background_color: hrColor,
		}),
		rule(".hr::before", {
			margin_right: vars.gap.or("0.5em"),
		}),
		rule(".hr::after", {
			margin_left: vars.gap.or("0.5em"),
		}),
	),
	font: group(
		rule([".em", ".italic", ".i"], { font_style: "italic" }),
		rule([".ltr", ".thinner"], { font_weight: "100" }),
		rule([".lt", ".thin"], { font_weight: "200" }),
		rule([".r", ".regular"], { font_weight: vars.font.weight.regular.or("400") }),
		rule([".sb", ".medium"], { font_weight: vars.font.weight.medium.or("500") }),
		rule([".b", ".bold"], { font_weight: vars.font.weight.bold.or("600") }),
		rule([".br", ".bb", ".bolder"], { font_weight: vars.font.weight.bolder.or("700") }),
		rule([".bst", ".boldest", ".bbb"], { font_weight: vars.font.weight.boldest.or("800") }),
		rule(".lh", { line_height: vars.line.height.or("1.25em") }),
		rule(".nolh", { line_height: "0em" }),
		rule(".lh-0", { __line_height: "0em" }),
		rule([".lh-1", ".lh-100"], { line_height: "1em" }),
		rule(".lh-125", { __line_height: "1.25em" }),
		rule(".lh-150", { __line_height: "1.5em" }),
		rule(".lh-175", { __line_height: "1.75em" }),
		rule(".lh-200", { __line_height: "2.00em" }),
		...times(10, (i) =>
			rule(`.t-${i}`, { font_size: `calc(1em * ${vars.text.size[i]})` }),
		),
		...times(10, (i) =>
			rule(`.t-${i}r`, { font_size: `calc(1rem * ${vars.text.size[i]})` }),
		),
		rule(".mono", { font_family: `${vars.font.mono}` }),
		rule(".sans", { font_family: `${vars.font.sans}` }),
		rule(".serif", { font_family: `${vars.font.serif}` }),
		rule(".code", { font_family: `${vars.font.code.family}` }),
		rule(".control", { font_family: `${vars.control.font.family}` }),
		rule(".heading", { font_family: `${vars.font.heading.family}` }),
		rule(".script", { font_family: `${vars.font.script.family}` }),
		rule(".display", { font_family: `${vars.font.display.family}` }),
		rule(".tight", { letter_spacing: "-0.05ch" }),
		rule(".tighter", { letter_spacing: "-0.10ch" }),
		rule(".tightest", { letter_spacing: "-0.15ch" }),
		rule([".unl", ".u"], { text_decoration: "underline" }),
		rule([".ovl", ".o"], { text_decoration: "overline" }),
		rule([".stk", ".striked", ".s"], { text_decoration: "line-through" }),
		rule(".hi", { background_color: vars.color.hi }),
		...Object.keys(sizenames).map((k, i) =>
			group(
				rule(`.t-${k}`, {
					font_size: `calc(1rem * ${vars.text.size[i]})`,
					line_height: vars.line.height.or("1.25em"),
				}),
				rule(`.${k}`, {
					font_size: `calc(1em * ${vars.text.size[i]})`,
					line_height: vars.line.height.or("1.25em"),
				}),
			),
		),
	),
});
// EOF
