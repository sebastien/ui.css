...
function* render(compact, values) {
	// yield `/* Generated by ui.css */`;
	for (const v of values) {
		if (v.constructor === Array) {
			for (const w of v) {
				yield* render(compact, [w]);
			}
		} else if (v.constructor === Object) {
			yield* render(compact, Object.values(v));
		} else if (v && typeof v.lines === "function") {
			for (const l of v.lines(compact, 0)) {
				yield l;
			}
		}
...
// Function: css
// Renders a readable stylesheet. Use css.compact for distribution output.
function* css(...values) {
	yield* render(false, values);
}

// Function: css.compact
// Renders stylesheet values without formatting-only whitespace or comments.
css.compact = function* (...values) {
	yield* render(true, values);
};
