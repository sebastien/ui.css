...
- Alerts: add `.alert` for styling and `role="alert"` for assistive technology, with `.success`, `.warning`, `.danger`, and `.error` variants. Semantic alert text, form `.error` text, and danger menu labels mix the semantic color toward the text pole in srgb, so the hue is preserved while the label meets AA on its tint in both modes.
...
- Pills and badges use the background color channel, so `.bg-*` color, tint, blend, and opacity modifiers apply to them. `.badge` is a pill-shaped text chip; add `.count` (or use `.count` alone) for a circular number counter whose size is `--badge-size`. Solid fills take a `contrast-color()` label (dark on neutral/green/amber, light on blue/red); `.tinted` and `.outline` keep the accent hue but mix the label toward the text pole, so variants meet AA in light and dark.
...
- Disclosure markers: the accordion uses a `›` chevron in a fixed square glyph box centered on both axes, so it rotates about its own visual center and points down when open. Size it with `--accordion-marker-size` / `--accordion-marker-font-size`, and move the pivot with `.origin-*` or `--motion-origin-x` / `--motion-origin-y` (an unset axis stays centered). The `.section` marker is a border-drawn chevron on the same origin channels.
